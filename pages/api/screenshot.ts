...
const Screenshot = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    await page.goto(`http://localhost:62591/sku/${sku}/${sku}`);

    const labels = await page.$$('.label');

    if (labels.length > 0) {
        const label = labels[0];
        await label.screenshot({  path:`public/sku/${sku}/${sku}_label_front.png` , omitBackground: true });
    } else {
        await browser.close();
    ...
    if (advert.length > 0) {
        ...
        await label.screenshot({ path:`public/sku/${sku}/${sku}_advert.png` });
    } else {
        await browser.close();
    ...
    await browser.close();
    return res.status(200).json({ message: `Screenshot taken of http://localhost:62591/sku/${sku}/${sku}` });
}
